...
class ProductCatalogueController extends Controller
{
     ...
     * @return Response
     */
    public function index($business_id, $location_id, $selling_Group, $category_id)
    //     public function index($business_id, $location_id)
    {

        $selling_Group_val = $selling_Group;
        ...
        if ($category_id == 0) {
            ...
            $products = Product::where('business_id', $business_id)->where('category_id', $category_id_val)
                ->whereHas('product_locations', function ($q) use ($location_id) {
                    $q->where('product_locations.location_id', $location_id);
                })
                ->ProductForSales()
        ...
        return view('productcatalogue::catalogue.index')->with(compact('products', 'business', 'discounts', 'business_location', 'categories', 'selling_Group_val'));
    ...
    public function generateQr()
    {
        ...
        $Selling_Groups = SellingPriceGroup::where('business_id', $business_id)->get();
        $Catagoreies = Category::where('business_id', $business_id)->get();

        return view('productcatalogue::catalogue.generate_qr')
            ->with(compact('business_locations', 'business', 'Selling_Groups', 'Catagoreies'));
    }
}
