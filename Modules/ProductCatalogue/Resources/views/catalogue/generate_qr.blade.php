        ...
        <div class="row">
            <div class="col-md-7">
                @component('components.widget', ['class' => 'box-solid'])
                    ...
                    {{-- Customer --}}
                    <div class="form-group">
                        {!! Form::label('subtitle', __('productcatalogue::lang.Selling') . ':') !!}
                        <select class="form-control" name="SellingGrop" id="selling_Group" required>
                         <option value="0">@lang( 'productcatalogue::lang.all_selling' )</option>
                            @foreach ($Selling_Groups as $Selling_Group)
                                <option value="{{ $Selling_Group->id }}">{{ $Selling_Group->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    
                       <div class="form-group">
                        {!! Form::label('subtitle', __('productcatalogue::lang.Catagoreie') . ':') !!}
                        <select class="form-control" name="Catagoreie" id="Catagoreie" required>
                            <option value="0">@lang( 'productcatalogue::lang.all_Catagoreie' )</option>
                            @foreach ($Catagoreies as $Catagoreie)
                                <option value="{{ $Catagoreie->id }}">{{ $Catagoreie->name }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="col-md-5">
                @component('components.widget', ['class' => 'box-solid'])
                    <div class="text-center">
                        ...
                        <br>
                        <a href="#" class="btn btn-success hide" id="download_image" target="_blank"
    ...
    <script type="text/javascript">
        (function($) {
            ...
            $(document).on('click', '#generate_qr', function(e) {
                ...
                $('#qrcode').html('');
                if ($('#location_id').val() && $('#selling_Group').val() ) {
                    var link = "{{ url('catalogue/' . session('business.id')) }}/" + $('#location_id').val() + '/' + $('#selling_Group').val() + '/' + $('#Catagoreie').val();
                    var color = '#000000';
                    if ($('#color').val().trim() != '') {
                        color = $('#color').val();
                    }
