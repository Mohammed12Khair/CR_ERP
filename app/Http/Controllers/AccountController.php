...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        // Create One array for the active accounts
        $userid = request()->session()->get('user.id');
        $active_accounts_IDs = Activeaccounts::where('userid', $userid)->where('business_id', $business_id)->get();
        $activeAccounts = [];
        foreach ($active_accounts_IDs as $active_accounts_ID) {
        ...
        if (request()->ajax()) {
            ...
            $accounts = Account::leftjoin('account_transactions as AT', 'AT.account_id', '=', 'accounts.id')
                ...
                ->whereIn('accounts.id', $activeAccounts)
                ->where('accounts.business_id', $business_id)
                ->select([
                    'accounts.name', 'accounts.account_number', 'accounts.note', 'accounts.id', 'accounts.account_type_id',
    ...
    public function cashFlow()
    {
        ...
        if (request()->ajax()) {
            ...
            return DataTables::of($accounts)
                ...
                ->removeColumn('id')
                ->rawColumns(['credit', 'debit', 'balance', 'sub_type', 'total_balance'])
                ->make(true);
        }
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if ($row->is_advance == 1) {
            $details .= '<br>(' . __('lang_v1.advance_payment') . ')<br>';
