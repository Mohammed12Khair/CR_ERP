...
class TransactionUtil extends Util
{
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($payment['method'] == 'cheque') {
                        ...
                        $add_Cheque->transaction_id = $transaction->id;
                        $add_Cheque->business_id = $transaction->business_id;
                        $add_Cheque->cheque_number = $payment['cheque_number'];
                        $add_Cheque->cheque_date = $payment['cheque_date'];
                        $add_Cheque->amount = $payment_amount;
                        $add_Cheque->transaction_type = $transaction->type;
                        $add_Cheque->comment = $payment['note'];
                        // $add_Cheque->userid=$transaction->created_by;
                        // $add_Cheque->payment_id = $payments_formatted['id'];
                        // $add_Cheque->save();
                        $payment_amount = 0;
                    }
     ...
     *
     */
    public function getLedgerDetails_old($contact_id, $start, $end)
    {
        $business_id = request()->session()->get('user.business_id');
        //Get sum of totals before start date
    ...
    public function createExpense_transfer($request, $business_id, $user_id, $format_data = true)
    {
        // location_id
        // final_total
        // expense_category_id
