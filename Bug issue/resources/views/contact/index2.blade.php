...
<section class="content">
    ...
    @slot('tool')
    <div class="box-tools">
        <button type="button" class="btn btn-block btn-primary btn-modal" data-href="{{action('ContactController@create', ['type' => $type])}}" data-container=".contact_modal">
            <i class="fa fa-plus"></i> @lang('messages.add')</button>
    </div>
    @endslot
    @endif
    @if(auth()->user()->can('supplier.view') || auth()->user()->can('customer.view') || auth()->user()->can('supplier.view_own') || auth()->user()->can('customer.view_own'))
    <table class="table table-bordered table-striped" id="contact_table2x" style="width:100%;">
        <thead>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                <td><a href="<?php echo action('ContactController@show',[$line->id]);?>" >{{$line->name}}</a></td>
                <td>{{$line->type}}</td>
                <td>{{$line->name}}</td>
                <td>{{$line->name}}</td>
