...
<section class="content-header">
    <h1>التعديل<i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('purchase.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h1>
    <table class="table table-bordered table-striped ajax_view" id="edit_table">
        ...
        <tbody>
            ...
            <tr>
                <td> <a href="<?php echo action('PurchaseController@edithistorydetails', ["id" => $onedata->id, "type" => $onedata->type]); ?>" class="btn btn-sm btn-success">show</a></td>
                <td>{{$onedata->updated_at}}</td>
                <td>{{$onedata->ref_no}}</td>
                <td><?php echo App\Contact::where('id', $onedata->contact_id)->first()->first_name; ?></td>
                <td>{{$onedata->final_total}}</td>
                <td>{{$onedata->type}}</td>
                <!-- <td></td> -->
            </tr>
            @endforeach
