        ...
        <table class="table table-bordered table-striped" id="users_tadble" style="text-align: center;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    @if (in_array($categorie->id, $user_active_categories))
                    <td>
                        <form action="{{action('ManageUserController@CategoryManagerDelete')}}" method="post">
                            @csrf
                            <input name="user_id" value="{{$id}}" type="text" readonly hidden>
                            <input name="category_id" value="{{$categorie->id}}" type="text" readonly hidden>
                            <button class="btn btn-sm btn-danger"><i class="glyphicon glyphicon-trash"></i></button>
                        </form>
                    </td>
                    @else
                    <td>
                        <form action="{{action('ManageUserController@CategoryManagerAdd')}}" method="post">
                            @csrf
                            <input name="user_id" value="{{$id}}" type="text" readonly hidden>
                            <input name="category_id" value="{{$categorie->id}}" type="text" readonly hidden>
